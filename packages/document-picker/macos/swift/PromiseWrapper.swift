...
class PromiseWrapper {
  ...
  func resolve(fromCallSite callsite: String, result: Any?) {
  ...
  func reject(fromCallSite callsite: String, code: String, message: String, error: Error?) {
    guard let rejecter = promiseReject else {
      print("cannot reject promise because it's null")
      return
    }
    let errorMessage = "RNDPPromiseWrapper: \(message)"
    resetMembers()
